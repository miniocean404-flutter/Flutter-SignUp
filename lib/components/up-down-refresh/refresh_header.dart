...
//  double get _indicatorExtent => widget.linkNotifier.refreshIndicatorExtent;
  ...
  String get _showText {
    ...
    if (widget.enableInfiniteRefresh) {
      if (_refreshState == RefreshMode.refreshed || _refreshState == RefreshMode.inactive || _refreshState == RefreshMode.drag) {
...
//        });
  ...
  Widget build(BuildContext context) {
    // 是否为垂直方向
    var isVertical = widget.axisDirection == AxisDirection.down || widget.axisDirection == AxisDirection.up;
    // 是否反向
    var isReverse = widget.axisDirection == AxisDirection.up || widget.axisDirection == AxisDirection.left;
    // 是否到达触发刷新距离
    overTriggerDistance = _refreshState != RefreshMode.inactive && widget.pulledExtent >= widget.refreshTriggerPullDistance;
  ...
  List<Widget> _buildContent(bool isVertical, bool isReverse) {
    return <Widget>[
      ...
      Expanded(
        ...
        child: Text(
          _showText,
          style: CustomAppFont.of(context).font12!.copyWith(color: CustomAppColor.of(context).cubeColor),
        ),
      ),
    ];
  }
