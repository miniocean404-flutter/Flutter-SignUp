...
class FoldableButton extends StatefulWidget {
  const FoldableButton({Key? key}) : super(key: key);

  @override
  State<FoldableButton> createState() => _FoldableButtonState();
}

class _FoldableButtonState extends State<FoldableButton> with TickerProviderStateMixin {
  bool isTapped = true;
  ...
  Widget buildPrimaryItem(IconData source) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: const Color(0XFFE95A8B).withOpacity(0.8), blurRadius: verticalPadding.value),
        ],
      ),
      child: Icon(
        source,
          ...
          return Stack(
            children: <Widget>[
              ...
              Align(
                  ...
                  child: Container(
                    padding: EdgeInsets.only(left: 38, bottom: verticalPadding.value),
                    child: getItem(
                      options.elementAt(3),
                    ),
                  )),
